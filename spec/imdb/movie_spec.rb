...
describe 'Imdb::Movie' do
  describe 'valid movie' do
    ...
    it 'finds the poster' do
      expect(subject.poster).to eq('https://images-na.ssl-images-amazon.com/images/M/MV5BYmY2ZGEwMTYtNjBmZS00OGE4LWEyMmUtNjAwMWUxZjVmZTRiXkEyXkFqcGdeQXVyNjU0OTQ0OTY@.jpg')
    end
    ...
    it 'finds number of votes' do
      expect(subject.votes).to be_within(100_000).of(620_900)
    end

    it 'finds the title', focus: true do
  ...
  describe 'with title that has utf-8 characters' do
    context 'WALL-E' do
      # WALL-E
      subject { Imdb::Movie.search('Leon').first }

      it 'returns the proper title' do
        expect(subject.title).to eq('Léon (1994)')
      end
    end
  end
end
